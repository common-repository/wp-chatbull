...
class Settings extends CP_AdminController {
    ...
    function generate_code() {
        ...
        if ($this->form_validation->run() === true) {
            ...
            if ($this->token->create()) {
                $response['result'] = 'success';
                $response['message'] = $this->lang->line('token_created');
            } else {
                $response['errors'] = $this->lang->line('process_error');
            }
        } else {
    ...
    function _domain_check($url, $id = 0) {
        ...
        if ($id) {
            ...
            $token = $this->token->get_single(array('id >' => 0));
            if ($token) {
                $this->form_validation->set_message('_domain_check', "You can create only one Chatbox Widget.");
                return FALSE;
            } else {
                return TRUE;
            }
